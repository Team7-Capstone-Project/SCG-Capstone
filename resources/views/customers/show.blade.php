    ...
    <x-slot name="header">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div class="flex items-center space-x-3">
                <a href="{{ route('customers.index') }}" class="group inline-flex items-center justify-center p-2 rounded-xl bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/60 text-slate-500 hover:text-scg-red dark:hover:text-red-400 hover:shadow-md transition-all duration-200">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path>
                    </svg>
                </a>
                <div>
                    <span class="text-xs font-bold text-scg-red uppercase tracking-wider">{{ __('Customer Details') }}</span>
            ...
            <div class="flex items-center space-x-2">
                <a href="{{ route('customers.edit', $customer) }}" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white text-sm font-semibold rounded-xl shadow-md hover:shadow-lg transition-all duration-200">
                ...
                @can('delete', $customer)
                    <form action="{{ route('customers.destroy', $customer) }}" method="POST" class="inline" onsubmit="return confirm('{{ __('Are you sure you want to delete this customer? This is a fatal action and will delete all associated shipments.') }}');">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-700 hover:to-rose-700 text-white text-sm font-semibold rounded-xl shadow-md hover:shadow-lg transition-all duration-200">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
        ...
        <div class="max-w-[95%] mx-auto sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="flex items-start space-x-3">
                            ...
                            <div>
                                ...
                                <p class="mt-1 text-base font-semibold text-slate-800 dark:text-slate-100">{{ $customer->contact_person ?? '-' }}</p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-start space-x-3">
                            ...
                            <div>
                                ...
                                <p class="mt-1 text-base font-semibold text-slate-800 dark:text-slate-100">{{ $customer->country ?? '-' }}</p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-start space-x-3 md:col-span-2">
                            ...
                            <div>
                                ...
                                <p class="mt-1 text-base text-slate-700 dark:text-slate-300 leading-relaxed">{{ $customer->address ?? '-' }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-gradient-to-br from-slate-900 to-slate-950 dark:from-slate-900/60 dark:to-slate-950/60 border border-slate-800/80 shadow-lg rounded-2xl p-6 text-white flex flex-col justify-between">
                        ...
                        <div class="space-y-5">
                            <div class="flex items-center space-x-3">
                                ...
                                <div>
                                    ...
                                    <a href="tel:{{ $customer->phone }}" class="text-sm font-semibold text-white hover:text-blue-400 transition-colors">{{ $customer->phone ?? '-' }}</a>
                                </div>
                            </div>
                            ...
                            <div class="flex items-center space-x-3">
                                ...
                                <div>
                                    ...
                                    <a href="mailto:{{ $customer->email }}" class="text-sm font-semibold text-white hover:text-blue-400 transition-colors break-all">{{ $customer->email ?? '-' }}</a>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-8 pt-4 border-t border-white/10 text-center">
                        ...
                        <span class="text-3xl font-extrabold text-white bg-gradient-to-r from-scg-red to-rose-500 bg-clip-text text-transparent">{{ count($customer->shipments) }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border border-slate-200/50 dark:border-slate-800/50 shadow-sm rounded-2xl p-6 transition-all duration-300">
                <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-6">
                    ...
                    <form method="GET" action="{{ route('customers.show', $customer) }}" class="flex flex-wrap items-end gap-3 bg-slate-50 dark:bg-slate-950 p-4 rounded-xl border border-slate-200/60 dark:border-slate-800/60">
                        <div>
                        ...
                        <div class="flex items-end gap-3">
                            ...
                            <div class="flex gap-2">
                                <button type="submit" class="inline-flex items-center px-4 py-1.5 border border-transparent text-xs font-bold rounded-lg text-white bg-gradient-to-r from-scg-red to-red-700 hover:from-red-700 hover:to-red-800 focus:outline-none shadow-md transition-all duration-200">
                                    {{ __('Filter') }}
                                </button>
                                @if(request('start_date') || request('end_date') || request('quick_filter'))
                                    <a href="{{ route('customers.show', $customer) }}" class="inline-flex items-center px-4 py-1.5 border border-slate-200 dark:border-slate-800 text-xs font-bold rounded-lg text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800 shadow-sm transition-all duration-200">
                                        {{ __('Reset') }}
                                    </a>
                                @endif
                            </div>
                        ...
                        <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-800">
                            <thead class="bg-slate-50 dark:bg-slate-900/60">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">{{ __('Supplier') }}</th>
